...
class Fixer(object):
    ...
    def get_rates(self, date):
        ...
        #Print the API's response for debugging
        print(f"API Response for {date.strftime('%Y-%m-%d')}: {data}")
        
        # Check if 'rates' exists in the response
        if 'rates' not in data:
            print(f"Error: No 'rates' found in the response for {date.strftime('%Y-%m-%d')}. Full response: {data}")
            return  # Exit if 'rates' key does not exist
        ...
        for currency in self.currencies:
            if currency in data['rates']:
                ...
                print(f"Warning: Currency '{currency}' not found in rates for {date.strftime('%Y-%m-%d')}.")
